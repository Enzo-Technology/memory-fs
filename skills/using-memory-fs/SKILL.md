...
Tools: `context_write`, `context_search`, `context_browse`, `context_read`, `context_delete`,
`context_link`, `context_backlinks`.
...
Before writing, `context_search` the topic first. If a close record exists, **update it**
(re-`context_write` the same namespace+key) rather than creating a near-duplicate.
`context_write` warns on near-duplicates — don't ignore the warning.

## Writing (`context_write`)
...
- **Link** related records with `[[wikilinks]]` in the body — these auto-extract into the
  backlink graph. Prefer this over `context_link`; reserve `context_link` for relationships
...
- `context_search <query>` — search by relevance (FTS5: phrases, `AND`/`OR`, `prefix*`).
- `context_browse kind=...` — no query: `index` (overview), `recent`, `hubs` (most-linked,
  usually most important), `orphans` (often stale), `tags` (vocabulary). Also use `browse`
  to filter by tag or namespace.
- `context_backlinks` — what references a record; often the fastest way to understand a topic.
- `context_read namespace key` — fetch one record when you already have its exact key.

## Deleting

`context_delete` only when the user asks to forget something or a record is clearly wrong.
